...
function parse(url: string, getUrl: GetUrlFn, xhrRequest: XhrRequest) {
  return (data: POCJson): Promise<PointCloudOctreeGeometry> => {
    const { offset, boundingBox, tightBoundingBox, originalTightBox } = getBoundingBoxes(data);
...
function getBoundingBoxes(
  data: POCJson,
): { offset: Vector3; boundingBox: Box3; tightBoundingBox: Box3, originalTightBox: Box3 } {
  ...
  let originalTightBox = new Box3(min, max);
  ...
  if (data.tightBoundingBox) {
	...
	originalTightBox = new Box3(new Vector3(lx, ly, lz), new Vector3(ux, uy, uz));
  }
  ...
  return { offset, boundingBox, tightBoundingBox, originalTightBox};
}
